...
def _open_usage_cycle(*, db: Session, subscription: BillingSubscription) -> UsageCycle:
    get_access_snapshot(session=db, user_id=subscription.user_id)
    return db.exec(
...
def test_process_stripe_webhook_customer_subscription_updated_persists_scheduled_cancellation(
    ...
    subscription_id = f"sub_{uuid.uuid4().hex}"
    customer_id = f"cus_{uuid.uuid4().hex}"
    cancel_at = int((datetime.now(timezone.utc) + timedelta(days=7)).timestamp())
    ...
    assert subscription.cancel_at == datetime.fromtimestamp(cancel_at, tz=timezone.utc)
...
def test_process_stripe_webhook_refund_created_resolves_invoice_from_charge_and_revokes_access(
    ...
    async def fake_stripe_request(method: str, path: str, **_: Any) -> dict[str, Any]:
        assert method == "GET"
        assert path == "/v1/charges/ch_refund_full"
        return {
            "id": "ch_refund_full",
            "object": "charge",
            "invoice": invoice_id,
...
def test_process_stripe_webhook_refund_revocation_stays_blocked_after_subscription_updated(
    ...
    async def fake_stripe_request(method: str, path: str, **_: Any) -> dict[str, Any]:
        assert method == "GET"
        assert path == "/v1/charges/ch_refund_sticky"
        return {
            "id": "ch_refund_sticky",
            "object": "charge",
            "invoice": invoice_id,
    ...
    refreshed_subscription = db.get(BillingSubscription, subscription.id)
    refreshed_cycle = db.get(UsageCycle, cycle.id)
    snapshot = get_access_snapshot(session=db, user_id=user.id)
...
def test_process_stripe_webhook_refund_created_pending_does_not_revoke_access(
    ...
    async def fake_stripe_request(method: str, path: str, **_: Any) -> dict[str, Any]:
        assert method == "GET"
        assert path == "/v1/charges/ch_refund_pending"
        return {
...
def test_process_stripe_webhook_refund_updated_succeeded_revokes_after_pending(
    ...
    async def fake_stripe_request(method: str, path: str, **_: Any) -> dict[str, Any]:
        assert method == "GET"
        assert path == "/v1/charges/ch_refund_transition"
...
def test_process_stripe_webhook_refund_failed_is_noop(
    ...
    async def fake_stripe_request(method: str, path: str, **_: Any) -> dict[str, Any]:
        assert method == "GET"
        assert path == "/v1/charges/ch_refund_failed"
        return {
            "id": "ch_refund_failed",
            "object": "charge",
            "invoice": invoice_id,
...
def test_process_stripe_webhook_charge_refunded_remains_compatible_and_idempotent_after_refund_updated(
    ...
    async def fake_stripe_request(method: str, path: str, **_: Any) -> dict[str, Any]:
        assert method == "GET"
        assert path == "/v1/charges/ch_refund_legacy"
        return {
            "id": "ch_refund_legacy",
            "object": "charge",
            "invoice": invoice_id,
