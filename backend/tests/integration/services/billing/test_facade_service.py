...
def test_create_checkout_session_for_trial_allows_missing_payment_method(
    ...
    monkeypatch.setattr(stripe_client, "_stripe_request", fake_stripe_request)
...
def test_create_checkout_session_allows_repurchase_after_refund_revocation(
    ...
    subscription.access_revoked_at = datetime.now(UTC)
...
def test_process_pending_customer_sync_tasks_async_succeeds(
    ...
    monkeypatch.setattr(stripe_client, "_stripe_request_raw", fake_stripe_request_raw)
...
def test_ambassador_usage_is_tracked_in_managed_monthly_cycle(
    ...
    assert reservation is not None
    event = usage_service.finalize_usage_reservation(
        session=db,
        request_key=request_key,
        quantity_consumed=4,
        utcnow_fn=lambda: now,
...
def test_managed_usage_resets_with_new_utc_month(
    ...
    assert summary.managed_access_source == "ambassador"
    assert summary.current_period_start == datetime(2026, 5, 1, 0, 0, tzinfo=UTC)
    assert summary.current_period_end == datetime(2026, 6, 1, 0, 0, tzinfo=UTC)
    assert reports_usage.used == 0
    assert len(managed_cycles) == 2
    assert managed_cycles[0].period_start == datetime(2026, 4, 1, 0, 0, tzinfo=UTC)
    assert managed_cycles[1].period_start == datetime(2026, 5, 1, 0, 0, tzinfo=UTC)
