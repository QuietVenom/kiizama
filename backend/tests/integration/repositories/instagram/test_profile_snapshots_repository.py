...
def test_profile_snapshots_crud_filters_profile_and_handles_invalid_ids(
    ...
    first = persist_snapshot_tree(
        db,
        ig_id="snap-crud-ig-1",
        username="snap_crud_creator_one",
        scraped_at=datetime(2024, 3, 1, tzinfo=UTC),
        post_code="POST_CRUD_1",
        reel_code="REEL_CRUD_1",
    )
    second = persist_snapshot_for_profile(
        db,
        profile_id=first["profile_id"],
        scraped_at=datetime(2024, 3, 2, tzinfo=UTC),
        post_code="POST_CRUD_2",
        reel_code="REEL_CRUD_2",
    )

    fetched = _run(get_profile_snapshot(db, first["snapshot"]["_id"]))
    latest_for_profile = _run(
        get_profile_snapshot_by_profile_id(db, first["profile_id"])
    )
    listed_by_ids = _run(
        list_profile_snapshots(
            db,
            skip=0,
            limit=10,
            snapshot_ids=[first["snapshot"]["_id"], "not-a-uuid"],
        )
    )
    updated = _run(
        update_profile_snapshot(
            db,
            first["snapshot"]["_id"],
            UpdateProfileSnapshot(metrics_id=None),
        )
    )
    replaced = _run(
        replace_profile_snapshot(
            ...
            ProfileSnapshot(
                profile_id=first["profile_id"],
                post_ids=[second["posts"]["_id"]],
                reel_ids=[second["reels"]["_id"]],
                metrics_id=second["metrics"]["_id"],
                scraped_at=datetime(2024, 3, 3, tzinfo=UTC),
            ),
        )
    )
    deleted = _run(delete_profile_snapshot(db, first["snapshot"]["_id"]))

    assert fetched is not None
    assert fetched["_id"] == first["snapshot"]["_id"]
    assert latest_for_profile is not None
    assert latest_for_profile["_id"] == second["snapshot"]["_id"]
    assert [snapshot["_id"] for snapshot in listed_by_ids] == [first["snapshot"]["_id"]]
    assert updated is not None
    assert updated["metrics_id"] is None
    assert replaced is not None
    assert replaced["post_ids"] == [second["posts"]["_id"]]
    assert deleted is not None
    assert deleted["_id"] == first["snapshot"]["_id"]
    assert _run(get_profile_snapshot(db, first["snapshot"]["_id"])) is None
    assert _run(get_profile_snapshot(db, "not-a-uuid")) is None
    assert _run(get_profile_snapshot_by_profile_id(db, "not-a-uuid")) is None
    assert _run(list_profile_snapshots(db, snapshot_ids=["bad-id"])) == []
...
def test_list_profile_snapshots_full_applies_limit_per_username(db: Session) -> None:
    first_profile_snapshot = persist_snapshot_tree(
        ...
        scraped_at=datetime(2024, 1, 3, tzinfo=UTC),
    ...
    persist_snapshot_for_profile(
        ...
        scraped_at=datetime(2024, 1, 2, tzinfo=UTC),
...
def test_list_profile_snapshots_full_uses_constant_query_count(db: Session) -> None:
    first_profile_snapshot = persist_snapshot_tree(
        ...
        scraped_at=datetime(2024, 2, 3, tzinfo=UTC),
    ...
    persist_snapshot_for_profile(
        db,
        profile_id=first_profile_snapshot["profile_id"],
        scraped_at=datetime(2024, 2, 2, tzinfo=UTC),
    ...
    persist_snapshot_tree(
        ...
        scraped_at=datetime(2024, 2, 4, tzinfo=UTC),
        post_code="POST_ZETA",
        reel_code="REEL_ZETA",
    )
    db.expunge_all()
