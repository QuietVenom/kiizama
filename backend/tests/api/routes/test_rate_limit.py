...
from app.core.redis import configure_redis_client_resolver
...
def test_password_recovery_enforces_ip_email_limit(client: TestClient) -> None:
    redis = EvalShaFakeRedis()
    configure_redis_client_resolver(lambda: redis)
...
def test_signup_returns_429_after_same_ip_limit(client: TestClient) -> None:
    redis = EvalShaFakeRedis()
    configure_redis_client_resolver(lambda: redis)
...
def test_public_auth_fail_closed_returns_503_when_redis_fails(
    ...
    redis = EvalShaFakeRedis()
    redis.raise_on_eval = RuntimeError("redis unavailable")
    configure_redis_client_resolver(lambda: redis)
