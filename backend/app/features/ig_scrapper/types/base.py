...
class BaseInstagramWorker:
    ...
    def safe_cast_to_list(obj: Any, default: list[Any] | None = None) -> list[Any]:
        if default is None:
            default = []
        return cast(list[Any], obj) if isinstance(obj, list) else default
    ...
    async def retryable_goto(self, page: Page, url: str, **kwargs) -> Response | None:
    ...
    async def retryable_click(self, page: Page, selector: str, **kwargs) -> bool:
    ...
    async def retryable_wait_for_selector(
        self, page: Page, selector: str, **kwargs
    ) -> ElementHandle | None:
