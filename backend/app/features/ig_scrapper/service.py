...
def _require_document(doc: Document | None, *, message: str) -> Document:
    if doc is None:
    ...
    for username, profile_result in response.results.items():
        ...
        try:
            ...
            post_doc = _require_document(
                post_doc,
                message=f"Failed to persist posts document for username '{username_value}'",
            )
            ...
            reel_doc = _require_document(
                reel_doc,
                message=f"Failed to persist reels document for username '{username_value}'",
            )
            ...
            overall_engagement_rate = float(
                _get_field_value(metrics_source, "overall_engagement_rate", 0.0) or 0.0
            )
            ...
            metrics_doc = _require_document(
                metrics_doc,
                message=f"Failed to persist metrics document for username '{username_value}'",
            )
