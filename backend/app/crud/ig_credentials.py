...
async def create_ig_credential(collection, credential: IgCredential):
...
async def get_ig_credential(collection, credential_id: str):
    return await collection.find_one({"_id": ObjectId(credential_id)})


async def list_ig_credentials(collection, skip: int = 0, limit: int = 100):
    cursor = collection.find().skip(skip).limit(limit)
    return [doc async for doc in cursor]


async def update_ig_credential(
    collection, credential_id: str, patch: UpdateIgCredential
):
...
async def update_ig_credential_session(
    collection, credential_id: str, session: dict[str, Any] | None
):
    return await collection.find_one_and_update(
        {"_id": ObjectId(credential_id)},
        {"$set": {"session": session}},
...
async def replace_ig_credential(
    collection, credential_id: str, credential: IgCredential
):
...
async def delete_ig_credential(collection, credential_id: str):
    return await collection.find_one_and_delete({"_id": ObjectId(credential_id)})
