...
def _normalize_snapshot_ids(snapshot_ids: list[str] | None) -> list[ObjectId] | None:
...
async def get_profile_snapshot(collection: Any, snapshot_id: str) -> Document | None:
    return cast(
        Document | None, await collection.find_one({"_id": ObjectId(snapshot_id)})
    )
...
async def get_profile_snapshot_by_profile_id(
    ...
    return cast(Document | None, await collection.find_one({"profile_id": profile_id}))
...
async def delete_profile_snapshot(collection: Any, snapshot_id: str) -> Document | None:
    return cast(
        Document | None,
        await collection.find_one_and_delete({"_id": ObjectId(snapshot_id)}),
    )
