...
test.describe("Blog routes", () => {
  ...
  test("renders the blog index from markdown content", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", {
        name: "Kiizama: de datos de creators a estrategias que si ayudan a decidir",
      }),
    ).toBeVisible()
    await expect(
      page.getByText(
  ...
  test("navigates from the blog index to a post detail page", async ({
    ...
    await expect(page).toHaveTitle(
      "Kiizama: inteligencia para creators, marcas y equipos de comunicacion",
    )
    await expect(page.locator('head meta[name="description"]')).toHaveAttribute(
      "content",
      "Descubre Kiizama, la plataforma que transforma datos de redes sociales en analisis, reportes y estrategias para creators, marcas, agencias y equipos de comunicacion.",
    )
    ...
    await expect(page.getByTestId("blog-post-content")).toContainText(
      "Kiizama es una plataforma de inteligencia que transforma datos de redes sociales en analisis",
    )
  })
